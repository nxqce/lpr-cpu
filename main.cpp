...
#define DEMO_PCIE_IO_ENABLE_ADDR		0x00
#define DEMO_PCIE_IO_WORD_ADDR		0x10
#define DEMO_PCIE_IO_STATUS_ADDR	0x20
...
int main() {
	...
	printf("== Terasic: LPR CPU-FPGA PCIe Program ==\r\n");
	...
	for (;;)
	{
		/// ----------Disable FPGA process---------
		BOOL bPass;
		int Mask = 0;
		bPass = PCIE_Write32(hPCIE, DEMO_PCIE_USER_BAR, DEMO_PCIE_IO_ENABLE_ADDR, (DWORD)Mask);
		...
		do {
			bPass = PCIE_Read32(hPCIE, DEMO_PCIE_USER_BAR, DEMO_PCIE_IO_STATUS_ADDR, &Status);
			printf("\n%d\n", Status);
